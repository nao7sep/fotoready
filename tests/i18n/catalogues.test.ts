...
// Keys whose text is genuinely the same word in that language as in English: format and brand
// names, words the language shares (German "Name", French "Description"), and the ASCII slug example
// where the language keeps slugs in English letters.
const SAME_AS_ENGLISH: Partial<Record<Language, readonly string[]>> = {
  de: [
    "common.ok", "renameComplete.name", "privacy.gps", "opName.hsl", "opName.lut", "opPicker.watermarkText",
    "output.format", "output.slug", "settings.tab.app", "settings.prompts", "settings.languageSystem",
  ...
  fr: [
    ...
    "output.description", "output.slug", "settings.avifEffort", "settings.prompts", "settings.confirmations",
    "metadataField.source", "metadataField.description", "metadataSummary.latitude", "metadataSummary.longitude",
    "metadataSummary.altitude", "rename.original", "about.version", "geometry.angle", "conceal.rectangle",
    "conceal.ellipse", "crop.original", "curves.point.other", "hsl.range.orange", "hsl.range.magenta", "hsl.global",
    "levels.gamma", "resize.exact", "strip.group.gps", "nativeMenu.services", "nativeMenu.zoom",
  ],
  it: [
  ...
  "pt-BR": [
    ...
    "metadataSummary.latitude", "metadataSummary.longitude", "metadataSummary.altitude", "rename.original",
    "crop.original", "hsl.range.magenta", "strip.group.gps", "nativeMenu.zoom",
  ],
  ru: ["privacy.gps", "opName.hsl", "opName.lut", "strip.group.gps"],
  ja: ["common.ok", "opName.hsl", "opName.lut", "output.slugPlaceholder", "about.issues", "strip.group.gps"],
  ko: ["opName.hsl", "opName.lut", "output.slugPlaceholder", "strip.group.gps"],
  "zh-Hans": ["opName.hsl", "opName.lut", "output.slugPlaceholder", "strip.group.gps"],
};
