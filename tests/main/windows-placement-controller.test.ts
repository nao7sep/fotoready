...
import { createWindowPlacementController, initializeWindowPlacement } from "@main/window-placement";
...
function configureWindowPlacement(win: Window, initial: WindowPlacementRecord,
  persist: (record: WindowPlacementRecord) => Promise<unknown>, onError: (error: unknown) => void,
  windows?: WindowsPlacement) {
  return createWindowPlacementController(win, {
    initialNormalBounds: initial.normalBounds, initialMode: initial.mode,
    initialWindowsNormalBounds: initial.windowsNormalBounds, windowsPlacement: windows, persist, onError,
  });
}
