...
try {
    ...
    Invoke-Native -FilePath "npm" -ArgumentList @("run", "check:imports")

    Write-Step "Running tests"
    Invoke-Native -FilePath "npm" -ArgumentList @("test")
    ...
    Write-Host ""
    Write-Host "fotoready update-packages failed: $($_.Exception.Message)" -ForegroundColor Red
    $scriptExitCode = 1
}
finally {
