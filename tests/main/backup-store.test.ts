/**
 * Tests for the write-through data-backup store (src/main/backup-store.ts) — fotoready's realization of the
 * data-backup conventions, mirroring the zipkit fleet reference. Pins the load-bearing guarantees:
 *
 ...
 *    prior rows untouched (the save it follows is unaffected); an unopenable store logs one warn and
 *    disables recording for the session (no repeat warn per save).
 *  - write-through: after a REAL managed save (saveSettings through the managed-text choke point), the exact
 *    bytes on disk are in the store, and a second identical save is deduped.
 *
...
type Captured = { message: string; fields?: LogFields };
const logCalls: { warn: Captured[]; error: Captured[] } = { warn: [], error: [] };
const capturingLogger = {
  debug() {},
  info() {},
  warn: (message: string, fields?: LogFields) => logCalls.warn.push({ message, fields }),
  error: (message: string, fields?: LogFields) => logCalls.error.push({ message, fields })
};
...
beforeEach(async () => {
  root = mkdtempSync(path.join(tmpdir(), "fotoready-backupstore-"));
  process.env[ENV_VAR] = root;
  logCalls.warn.length = 0;
  logCalls.error.length = 0;
  const { setBackupLogger } = await import("@main/backup-store");
  setBackupLogger(capturingLogger);
});
...
describe("record: BLOB fidelity, hash, size, path, and timestamp shape", () => {
  it("stores byte-identical content (CR/LF + non-UTF-8 byte), correct sha256, size, absolute path, ISO-ms time", async () => {
    ...
    // A UTF-8 BOM, a CR/LF pair, and a lone 0xFF (invalid UTF-8) — reading this as a string then storing it
    // would normalize the CR/LF, alter the BOM, or corrupt the 0xFF. The BLOB must be verbatim.
...
describe("best-effort: a record failure never throws, logs one warn, and does not disturb prior rows", () => {
  it("swallows an injected insert failure, logs exactly one warn, and leaves the earlier row intact", async () => {
    ...
    {
      const { record, setBackupLogger } = await import("@main/backup-store");
      setBackupLogger(capturingLogger);
    ...
    // Exactly one warn, naming the file and carrying an error; no error line.
  ...
  it("logs one warn and disables recording for the session when the store cannot be opened", async () => {
    ...
    process.env[ENV_VAR] = path.join(blocker, "nested"); // parent is a file -> mkdir/open fails
...
describe("write-through: a real managed save records the exact bytes after the rename", () => {
  it("saveSettings records config.json's exact on-disk bytes into the store", async () => {
    ...
    const file = path.join(root, "config.json");
    await saveSettings(file, { ...defaultGlobalSettings(null), defaultWebpQuality: 71 });
