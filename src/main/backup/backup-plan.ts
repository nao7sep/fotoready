 ...
 * granularity (imported LUTs/stamps may live on a USB stick); it costs nothing in missed edits because
 * the recorded time is from a prior backup run, which any real edit moves well beyond two seconds past.
 */
...
/** The latest entry per archive path. `archivedAt` is a `yyyymmdd-hhmmss(-fff)-utc` stamp (millisecond
 *  form going forward; older entries stay at second precision), so ordinal string comparison is
 *  chronological. */
