...
export function normalizeGlobalSettings(input: unknown, fallback: GlobalSettings): SettingsNormalizationResult {
  ...
  const settings: GlobalSettings = {
    theme: readValue(source, "theme", fallback.theme, issues, (value, path) => assertOneOf(value, path, ["dark", "light", "system"])),
    language: readValue(source, "language", fallback.language, issues, (value, path) => assertOneOf(value, path, ["en"])),
    cameraTimezone: readValue(source, "cameraTimezone", fallback.cameraTimezone, issues, assertNonEmptyString),
    ...
    checkForUpdates: readValue(source, "checkForUpdates", fallback.checkForUpdates, issues, assertBoolean),
    telemetry: false,
    ...
    workingColorSpace: readValue(source, "workingColorSpace", fallback.workingColorSpace, issues, (value, path) => assertOneOf(value, path, ["srgb"])),
    assumptionWhenNoIccNoTag: readValue(source, "assumptionWhenNoIccNoTag", fallback.assumptionWhenNoIccNoTag, issues, (value, path) => assertOneOf(value, path, ["srgb", "adobe-rgb"])),
    ...
    maxConcurrent: readValue(source, "maxConcurrent", fallback.maxConcurrent, issues, (value, path) => assertFiniteNumber(value, path, { integer: true, min: 1, max: 16 })),
