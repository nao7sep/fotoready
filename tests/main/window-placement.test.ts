...
import { applyRestoredWindowBounds, createWindowPlacementController } from "@main/window-placement";
...
function setup(
  ...
  const controller = createWindowPlacementController(win, { initialNormalBounds: win.getBounds(), initialMode: mode, persist, onError });
...
describe("applying restored bounds", () => {
  it("accepts an OS-adjusted rectangle without a correction loop", () => {
    ...
    const onError = vi.fn();
    expect(applyRestoredWindowBounds(win, target, onError)).toBe(true);
  ...
  it("contains an actual setter failure and uses the opening bounds", () => {
    ...
    const onError = vi.fn();
    expect(applyRestoredWindowBounds(win, { x: 50, y: 60, width: 1300, height: 850 }, onError)).toBe(false);
