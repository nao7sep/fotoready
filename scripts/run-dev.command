...
RENDERER_URL="http://127.0.0.1:23339"
...
pause_on_failure() {
  ...
  if [[ "$status" -ne 0 && ( "$status" -lt 128 || "$status" -gt 143 ) ]]; then
    echo
    echo "fotoready run-dev failed with exit code $status."
...
cleanup() {
  ...
  if node "$RUNTIME_HELPER" is-owner "$RUNTIME_TOKEN" >/dev/null 2>&1; then
    node "$RUNTIME_HELPER" stop-if-owner "$RUNTIME_TOKEN" electron "FotoReady" "FotoReady" >/dev/null 2>&1 || true
  fi
...
log_step "Replacing any existing FotoReady runtime"
node "$RUNTIME_HELPER" claim "$RUNTIME_TOKEN"
node "$RUNTIME_HELPER" stop electron "FotoReady" "FotoReady"
node "$RUNTIME_HELPER" check-endpoint 127.0.0.1 23339
...
log_step "Starting FotoReady in development mode"
npm run dev &
DEV_PID=$!
...
log_step "FotoReady is ready at $RENDERER_URL"
wait "$DEV_PID"
