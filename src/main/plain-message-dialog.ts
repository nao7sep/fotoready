...
export interface PlainMessageDialogOptions {
  ...
  message: string;
  detail?: string;
  /** The page's own words beyond the message, in the interface language, and that language's tag. */
  closeLabel: string;
  detailsLabel: string;
  lang: string;
}
...
export function renderPlainMessageDialogHtml(options: PlainMessageDialogOptions): string {
  ...
  </style></head><body><main class="dialog"><header class="header" id="dialog-header"><h1>${escapeHtml(options.title)}</h1></header><section class="body" id="dialog-body" role="region" aria-label="${escapeHtml(options.detailsLabel)}" tabindex="0"><p>${escapeHtml(options.message)}</p>${options.detail ? `<p class="detail">${escapeHtml(options.detail)}</p>` : ""}</section><footer class="actions" id="dialog-footer"><button id="close" class="button" type="button" onclick="location.href='${CLOSE_URL}'">${escapeHtml(options.closeLabel)}</button></footer></main></body></html>`;
}
