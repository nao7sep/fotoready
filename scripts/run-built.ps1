...
try {
    ...
    & node $runtimeHelper stop electron "FotoReady" "FotoReady"
    if ($LASTEXITCODE -ne 0) { throw "Could not replace the existing FotoReady runtime." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "FotoReady did not start." }
}
catch {
    Write-Host ""
    Write-Host "fotoready run-built failed: $($_.Exception.Message)" -ForegroundColor Red
    $scriptExitCode = 1
}
finally {
